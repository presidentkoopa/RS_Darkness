...
The engine applies darkness AFTER the lighting equation but BEFORE glow and
sweep (main.fp:2884), because those are emissive -- light being added rather
than light the room has.
...
SHOWCASE -- every one uses distance from the eye or height above the ground.
A sector holds ONE light number and cannot say where in it you are standing,
so none of these were possible per sector at any setting.

                curve      amt   dist  range   height  h-range  h-ref
  Horizon       subtract    32   0.90   1400      -        -       -
  Tunnel        subtract    48   0.95    380      -        -       -
  Abyss         subtract   256   1.00    850      -        -       -
  Undertow      subtract    72     -       -    0.95     140     feet
  Silt          subtract    40     -       -    0.90     700    fixed 64
  Nightfall     compress   150   0.45   1600      -        -       -
  Overcast      cap        150   0.30   2000      -        -       -
  Crush         gamma      175     -       -      -        -       -
  Cavern        gamma      150   0.65   1300    0.55     320     feet
  ...
  Abyss      true black plus range, which eats the EMISSIVE light on top --
             a glow across the room becomes a hint rather than a landmark
  Undertow   height tracking your feet, waist-deep wherever you walk
  Silt       fixed low reference, long range: a floor-to-ceiling gradient
             you can genuinely climb out of
...
Both calls are clearscope, pushed from UiTick as well as WorldTick, so sliders
move the picture while the game is paused and while the menu is open. No
network event.
...
-----
  ...
  zscript/rsd_presets.zs   four compatibility presets, nine showcase
  zscript/rsd_handler.zs   pushes two engine calls

SetDarkness and SetDarknessSpace were already exported -- the mod itself needs
no engine change.

Two engine fixes DO belong to this feature, and live in UZDXREMA rather than
here, because darkness reached things that were never room light:
...
Both follow the rule the engine already applies to glow: darkness scales the
light a room HAS, and emissive light is not room light. Requires an engine
rebuild.
